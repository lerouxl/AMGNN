...
class DoubleHeadSimpleMlp(torch.nn.Module):
    """"""
    def __init__(self, in_channels: int, hidden_channels: int, out_channels: int, number_hidden: int):
        """Simple MLP.
        ...
        number_hidden: int
            Number of hidden layer of the MLP
        """
        super(DoubleHeadSimpleMlp, self).__init__()
        ...
        self.network1 = MLP(in_channels=self.in_channels, hidden_channels=self.hidden, act="GELU",
                           out_channels=1, num_layers=self.number_hidden)
        self.network2 = MLP(in_channels=self.in_channels, hidden_channels=self.hidden, act="GELU",
    ...
    def forward(self, batch: Data) -> Tensor:
        ...
        x_1 = self.network1(batch.x)
        x_2 = self.network2(batch.x)
